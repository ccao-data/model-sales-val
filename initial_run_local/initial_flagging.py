...
df_final = (
    ...
    # PTAX-203 binary
    .assign(sv_is_ptax_outlier=lambda df: np.where(df["sv_outlier_type"] == "PTAX-203 flag", 1, 0))
...
# Means Table
unique_groups = df_final.drop_duplicates(subset=inputs["stat_groups"], keep="first").reset_index(
    drop=True
)
